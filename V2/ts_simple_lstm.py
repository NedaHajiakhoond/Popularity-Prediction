...
def monthly_plotting(Datatest,A,L,w):
	...
	for t in range(0,A.shape[0]):
		B[0,t] = mean_squared_error(Datatest[t,:], A[t,:])/A.shape[1]
		B[1,t] = mean_squared_error(Datatest[t,:], L[t,:])/A.shape[1]
	#print(B)
...
def repo_plotting(Datatest,A,L,w):
	print(np.shape(A))
	print(np.shape(L))
	print(np.shape(Datatest))
	B = np.zeros((2,A.shape[1]))
	for r in range(0,A.shape[1]):
		B[0,r] = mean_squared_error(Datatest[:,r], A[:,r])/A.shape[0]
		B[1,r] = mean_squared_error(Datatest[:,r], L[:,r])/A.shape[0]
	#print(B)
	plt.plot(range(1,A.shape[1]),B[0,1:] ,label='Moving average', color='blue', linestyle='dashed', marker='o', markerfacecolor='blue', markersize=5,)
...
'''
#np.random.seed(7)
Dir = '/home/social-sim/Documents/SocialSimCodeTesting/Pop_MLearning/10days166-100repos-fork/'
Fls = os.listdir(Dir)
Data = []
print(len(Fls))
...
Data = np.array(Data)
#Data=Data[:,[3,5,8,13,17,19,23,24,20,28,36,38,30,43,44,48,56,57,95,99]]
#Data=Data[:,[8,17,24,20,28,38,30,44,99]]
#Data=Data[:,[3,5,19,28,36,48,56,57]]
#Data=Data[:,[38,95,96,99]]
'''

data = pd.read_csv("/home/social-sim/Documents/SocialSimCodeTesting/Elastic_Search/Nisha/Fork-Watch/top_1000_all_event_repos-Fork-Watch-weekly.csv", nrows=100)
data_fork = data.loc[:,'f-start_2014-12-29.csv':'f-start_2017-08-28.csv']
data_watch = data.loc[:,'w-start_2014-12-29.csv':'w-start_2017-08-28.csv']
print(len(data_fork))
Data = []
Data_watch = []
Data_fork =[]
Data_fork = np.transpose(data_fork.values)
Data_watch = np.transpose(data_watch.values)
data_plotting(Data_fork)
sys.exit()
mv_errors=[]
ls_errors=[]
window_size=12
...
for w in range(1,window_size):
	...
	#repo_plotting(Datatest[w+1:,:],np.transpose(mv_d),ls_d,w)

print(ls_errors)
plt.plot(range(2,window_size-1),mv_errors[2:len(mv_errors)],label='Moving average', color='blue', linestyle='dashed', marker='o', markerfacecolor='blue', markersize=5,)
plt.plot(range(2,window_size-1),ls_errors[2:len(ls_errors)],label='LSTM', color='blue', linestyle='dashed', marker='o', markerfacecolor='red', markersize=5,)
plt.ylabel('RMSE')
