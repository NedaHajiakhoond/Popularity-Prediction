...
def simple_lstm(train,test,Datatrain,Datatest,ws):
	...
	model.fit(X, y, epochs=1*10000, shuffle=False, verbose=0)
	'''
...
def monthly_plotting(Datatest,A,L,Ari,w):
	...
	for t in range(0,A.shape[0]):
		...
		B[2,t] = mean_squared_error(Datatest[t,:], Ari[t,:])/A.shape[1]
...
def repo_plotting(Datatest,A,L,Ari,w):
	...
	for r in range(0,A.shape[1]):
		...
		B[2,r] = mean_squared_error(Datatest[:,r], Ari[:,r])/A.shape[0]
...
def arima_(org_data,train_size,p=5,d=1,q=0):
	from statsmodels.tsa.arima_model import ARIMA
	from pyramid.arima import auto_arima
	Data =org_data
	#for i in range(org_data.shape[0]):
	#	result = seasonal_decompose(org_data[i,:], model='additive')
	#	Data.append(result)
	stepwise_model = auto_arima(Data[1,:], start_p=1, start_q=1,
	...
	trainX=np.transpose(Datatrain)
	testX=np.transpose(Datatest)
...
Data = np.array(Data)
print(Data.shape)
ari = arima_(np.transpose(Data),10)
