...
def run():
	# load dataset
	df = read_csv('/home/social-sim/Downloads/sales-of-shampoo-over-a-three-ye.csv')
	df['extra']=df['1']
	df.to_csv('t.csv')
	series = read_csv('t.csv', header=0, parse_dates=[0], index_col=1, squeeze=True)
	series=series[['1','extra']]
	# experiment
	repeats = 10
	results = DataFrame()
	# run experiment1dlsw
