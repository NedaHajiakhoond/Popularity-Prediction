...
def simple_lstm(train,test,ts):
	...
	X = X.reshape(  X.shape[0],ts, X.shape[1])
	y = y.reshape(  y.shape[0],ts, y.shape[1])
		
	testX = testX.reshape(  testX.shape[0],ts, testX.shape[1])
	testy = testy.reshape(  testy.shape[0],ts, testy.shape[1])
	...
	model.fit(X, y, epochs=1*10, shuffle=False, verbose=1)
	'''
...
errors=[]
ls_error=[]
for ts in range(1,ts_size):
	ls_error.append(simple_lstm(train,test,ts))
	errors.append(mean_squared_error(Data[:,ts-1:], moving_average(Data,ts))/Data.shape[1])
#mv=moving_average(Data)
#ls=moving_average(Data*1.5)
#error_plotting(Data,mv,ls)
#data_plotting(Data)
plt.plot(range(2,ts_size),errors)
plt.plot(range(2,ts_size),ls_error)
plt.ylabel('MSE')
